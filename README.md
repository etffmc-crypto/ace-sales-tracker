...
1. Copy `.env.example` to `.env` and fill in `DATABASE_URL` (see below), `AUTH_SECRET`, `SEED_ADMIN_EMAIL`, `SEED_ADMIN_PASSWORD`, `ANTHROPIC_API_KEY`.
2. `npm install`
...
4. **Set environment variables in Vercel:** in the project's Settings → Environment Variables, add `DATABASE_URL`, `AUTH_SECRET`, `SEED_ADMIN_EMAIL`, `SEED_ADMIN_PASSWORD`, `ANTHROPIC_API_KEY` with the same values as your local `.env`.
